...
// Find all table definitions by properly tracking brackets
function extractTables(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Match table definitions (indented inside defineSchema)
    ...
    if (tableMatch) {
      ...
      for (let j = i; j < lines.length; j++) {
        ...
        // If we've found the matching bracket, check for indexes
        if (foundOpen && bracketCount === 0) {
          // Look for .index() calls on subsequent lines
          for (let k = j + 1; k < lines.length && k < j + 10; k++) {
            const indexLine = lines[k];
            if (indexLine.match(/^\s+\.index\(/)) {
              definition += indexLine + '\n';
            } else if (indexLine.match(/^\s+\}/)) {
              // End of table definition
              break;
            } else if (indexLine.trim() === '') {
      ...
      tables.push({ name, definition: definition.trim() });
    }
  }
...
for (const tableName of knownTables) {
  ...
  if (found) {
    console.log(`✓ Found ${tableName} (${found.definition.split('\n').length} lines)`);
  } else {
