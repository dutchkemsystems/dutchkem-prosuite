...
import { examCareerAgent } from "./exam_career_agent";
...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await examCareerAgent.agents[0].createThread(ctx, {
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        ...
        const { messageId } = await examCareerAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        ...
        if (exchangeCount < MAX_FREE_EXCHANGES) {
          ...
          await (examCareerAgent.agents[0] as any).answer(ctx, {
            threadId,
            promptMessageId: messageId,
            assistantId: (examCareerAgent.agents[0] as any).agentId ?? "exam-career-agent",
            text: response,
          });
    ...
    const composioAgentId = AGENT_ID_MAP["exam_career_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.exam_career_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await examCareerAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await examCareerAgent.streamWithFallback(ctx, threadId, messageId);
