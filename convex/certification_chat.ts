...
import { certificationAgent } from "./certification_agent";
...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await certificationAgent.agents[0].createThread(ctx, {
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        const { messageId } = await certificationAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        });
        await (certificationAgent.agents[0] as any).answer(ctx, {
          threadId,
          promptMessageId: messageId,
          assistantId: (certificationAgent.agents[0] as any).agentId ?? "certification-agent",
    ...
    const composioAgentId = AGENT_ID_MAP["certification_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.certification_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await certificationAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await certificationAgent.streamWithFallback(ctx, threadId, messageId);
