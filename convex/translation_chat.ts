...
import { translationAgent } from "./translation_agent";
...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await translationAgent.agents[0].createThread(ctx, {
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        ...
        const { messageId } = await translationAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        ...
        if (exchangeCount < MAX_FREE_EXCHANGES) {
          ...
          await (translationAgent.agents[0] as any).answer(ctx, {
            threadId,
            promptMessageId: messageId,
            assistantId: (translationAgent.agents[0] as any).agentId ?? "translation-agent",
            text: response,
          });
    ...
    const composioAgentId = AGENT_ID_MAP["translation_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.translation_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await translationAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await translationAgent.streamWithFallback(ctx, threadId, messageId);
