...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await travelAgent.agents[0].createThread(ctx, {
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        const { messageId } = await travelAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        });
        await (travelAgent.agents[0] as any).answer(ctx, {
          threadId,
          promptMessageId: messageId,
          assistantId: (travelAgent.agents[0] as any).agentId ?? "travel-agent",
    ...
    const composioAgentId = AGENT_ID_MAP["travel_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.travel_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await travelAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await travelAgent.streamWithFallback(ctx, threadId, messageId);
