...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await contentAgent.agents[0].createThread(ctx, {
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        const { messageId } = await contentAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        });
        await (contentAgent.agents[0] as any).answer(ctx, {
          threadId,
          promptMessageId: messageId,
          assistantId: (contentAgent.agents[0] as any).agentId ?? "content-agent",
    ...
    const composioAgentId = AGENT_ID_MAP["content_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.content_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await contentAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await contentAgent.streamWithFallback(ctx, threadId, messageId);
