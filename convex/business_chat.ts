...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await businessAgent.agents[0].createThread(ctx, {
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        ...
        const { messageId } = await businessAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        ...
        if (exchangeCount < MAX_FREE_EXCHANGES) {
          ...
          await (businessAgent.agents[0] as any).answer(ctx, {
            threadId,
            promptMessageId: messageId,
            assistantId: (businessAgent.agents[0] as any).agentId ?? "business-agent",
            text: response,
          });
    ...
    const composioAgentId = AGENT_ID_MAP["business_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.business_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await businessAgent.streamWithFallback(ctx, threadId, messageId);
