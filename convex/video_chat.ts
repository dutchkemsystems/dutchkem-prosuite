...
import { videoAgent } from "./video_agent";
...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await videoAgent.agents[0].createThread(ctx, {
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        const { messageId } = await videoAgent.agents[0].saveMessage(ctx, {
          threadId,
          prompt,
          userId,
          skipEmbeddings: true,
        });
        await (videoAgent.agents[0] as any).answer(ctx, {
          threadId,
          promptMessageId: messageId,
          assistantId: (videoAgent.agents[0] as any).agentId ?? "video-agent",
    ...
    const composioAgentId = AGENT_ID_MAP["video_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.video_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await videoAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await videoAgent.streamWithFallback(ctx, threadId, messageId);
