...
import { shoppingAgent } from "./shopping_agent";
...
export const createThread = mutation({
  ...
  handler: async (ctx) => {
    ...
    const { threadId } = await shoppingAgent.agents[0].createThread(ctx, {
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    const composioAgentId = AGENT_ID_MAP["shopping_chat"];
    ...
    await ctx.scheduler.runAfter(0, internal.shopping_chat.generateResponse, {
      threadId,
      promptMessageId: messageId,
    });
    return messageId;
...
export const generateResponse = internalAction({
  ...
  handler: async (ctx, { promptMessageId, threadId }) => {
    const userId = await getAuthUserId(ctx);
    await shoppingAgent.streamWithFallback(
      ctx,
      threadId,
      promptMessageId,
      userId ?? undefined
...
export const generateSimpleResponse = internalAction({
  ...
  handler: async (ctx, { prompt }) => {
    ...
    await shoppingAgent.streamWithFallback(ctx, threadId, messageId);
