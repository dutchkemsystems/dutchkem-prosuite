  ...
  handler: async (ctx, { prompt, threadId }) => {
    ...
    if (userId) {
      ...
      if (!sub.active) {
        ...
        await (financeAgent.agents[0] as any).answer(ctx, {
          threadId,
          promptMessageId: messageId,
          assistantId: (financeAgent.agents[0] as any).agentId ?? "finance-agent",
    ...
    const composioAgentId = AGENT_ID_MAP["finance_chat"];
