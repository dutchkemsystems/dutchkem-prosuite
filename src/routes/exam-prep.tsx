        ...
        {results.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 w-full">
              <QuickAction icon="🎯" text="PMP Practice" />
              <QuickAction icon="☁️" text="Cloud Cert Prep" />
              <QuickAction icon="📊" text="GRE/GMAT Help" />
              <QuickAction icon="🛡️" text="Security+ Tips" />
            </div>
          </div>
        )}
    ...
    <div className={`flex ${isAssistant ? 'justify-start' : 'justify-end'} group animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      <div className={`max-w-[85%] md:max-w-[70%] flex gap-3 ${isAssistant ? '' : 'flex-row-reverse'}`}>
        <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center text-xl font-bold ${
          isAssistant ? 'bg-gradient-to-br from-yellow-500 to-orange-500 text-white' : 'bg-slate-800 border border-slate-700 text-white'
        }`}>
          {isAssistant ? '📝' : '👤'}
        </div>
        <div className={`p-5 rounded-2xl ${
          isAssistant 
            ...
            : 'bg-gradient-primary text-white shadow-lg shadow-orange-500/10'
        }`}>
