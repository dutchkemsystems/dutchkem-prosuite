...
function FinanceAdvisorPage() {
  ...
  const createThread = useMutation(api.finance_chat.createThread);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans">
      <AgentHeader agentName="Finance Advisor" />
...
function ChatContainer({ threadId }: { threadId: string }) {
  const { results, status, loadMore } = useUIMessages(
    api.finance_chat.listMessages,
    { threadId },
    { initialNumItems: 50, stream: true },
  );
  const sendMessage = useMutation(api.finance_chat.sendMessage).withOptimisticUpdate(
    optimisticallySendMessage(api.finance_chat.listMessages),
  );
    ...
    <div className="flex-grow flex flex-col h-full">
      ...
      >
        ...
        {results.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-6">
            <div className="w-20 h-20 bg-gradient-to-br from-emerald-500 to-green-600 rounded-3xl flex items-center justify-center text-5xl shadow-2xl shadow-emerald-500/20">💰</div>
            <div>
              <h3 className="text-2xl font-bold text-white mb-2 uppercase tracking-tight">Wealth Creation Hub</h3>
              <p className="text-slate-400 font-medium leading-relaxed text-sm">Achieve financial freedom. I'm here to help you budget, invest wisely, and eliminate debt forever.</p>
            </div>
            <div className="grid grid-cols-2 gap-3 w-full">
              <QuickAction icon="📉" text="Debt Repayment" onClick={() => handleQuickAction("Help me create a debt repayment plan")} />
              <QuickAction icon="📊" text="Risk Profiling" onClick={() => handleQuickAction("Assess my investment risk profile")} />
              <QuickAction icon="🏠" text="Goal Planning" onClick={() => handleQuickAction("Help me plan my financial goals")} />
              <QuickAction icon="🏖️" text="Retirement Path" onClick={() => handleQuickAction("Create a retirement savings plan for me")} />
            </div>
          </div>
        )}
      ...
      <div className="p-4 md:p-8 pt-0">
        <div className="flex items-center gap-2">
          ...
          <AgentSupportButton agentId="A7" agentName="Finance Pro" />
        </div>
        <p className="text-[10px] text-center text-slate-600 mt-3 uppercase tracking-widest font-black">
          Financial Engineering by ProSuite NG+ • Wealth Intelligence
        </p>
      </div>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="relative group">
      <input
        ...
        placeholder="What are your financial goals today?"
