  ...
  return (
    <div className={`flex ${isAssistant ? 'justify-start' : 'justify-end'} group animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      <div className={`max-w-[85%] md:max-w-[70%] flex gap-3 ${isAssistant ? '' : 'flex-row-reverse'}`}>
        <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-sm font-bold ${isAssistant ? 'bg-gradient-to-br from-red-600 to-orange-500 text-white' : 'bg-red-600 text-white'}`}>{isAssistant ? '🎬' : 'U'}</div>
        <div className={`p-4 rounded-2xl ${isAssistant ? 'bg-slate-800 text-slate-100 border border-slate-700 shadow-sm' : 'bg-red-600 text-white shadow-lg shadow-red-500/10'}`}>
          <div className="whitespace-pre-wrap leading-relaxed text-sm">{visibleText}{message.status === "streaming" && <span className="inline-block w-1.5 h-4 ml-1 bg-red-400 animate-pulse align-middle"></span>}</div>
