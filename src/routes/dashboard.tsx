      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-700 text-white">
        <div className="text-center max-w-md p-8">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={() => navigate({ to: '/auth' })} className="px-6 py-3 bg-slate-700 rounded-xl text-white font-bold hover:bg-slate-700 transition-colors border border-slate-500">
              Back to Sign In
            </button>
          </div>
        </div>
...
function DashboardContent() {
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-700 text-white">
        <div className="text-center max-w-md p-8">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={async () => { try { await signOut(); } catch {} navigate({ to: '/auth' }); }} className="px-6 py-3 bg-slate-700 rounded-xl text-white font-bold hover:bg-slate-700 transition-colors border border-slate-500">
              Sign In Again
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col md:flex-row overflow-hidden relative">
      ...
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between p-4 bg-slate-700 border-b border-slate-500 sticky top-0 z-30">
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="w-12 h-12 flex items-center justify-center rounded-xl bg-slate-700 text-white" aria-label="Toggle menu">
          {sidebarOpen ? '✕' : '☰'}
        </button>
      </div>

      {/* Navigation Sidebar */}
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static top-0 left-0 h-full md:h-auto w-72 md:w-64 bg-slate-700 border-r border-slate-500 flex-shrink-0 z-20 flex flex-col transition-transform duration-300 md:transition-none`}>
        <div className="p-4 md:p-6">
          <Link to="/" className="hidden md:flex items-center gap-3 mb-8 group">
            <CompanyLogo className="w-12 h-12" />
            <div className="flex flex-col">
              <span className="font-black text-xs tracking-tighter uppercase leading-none text-white">Ventures ProSuite NG+</span>
              <span className="text-xs font-bold text-white uppercase tracking-widest mt-1">RC: 9489855</span>
            </div>
          </Link>
        ...
        <div className="mt-auto p-4 md:p-6 border-t border-slate-500">
      ...
      <main className="flex-grow overflow-y-auto h-screen flex flex-col">
        ...
        {modal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" onClick={() => setModal(null)}></div>
            <div className="relative bg-slate-700 border border-slate-500 w-full max-w-lg rounded-3xl p-8 shadow-2xl ">
              <button onClick={() => setModal(null)} className="absolute top-6 right-6 text-white hover:text-white text-xl">✕</button>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      return (
                        <button key={agent.agentId} onClick={() => navigate({ to: agent.path })} className="p-4 bg-slate-700 rounded-2xl border border-slate-500 hover:border-indigo-500 transition-all text-left relative">
                          ...
                          {enhancement?.enhanced && <div className="text-xs text-emerald-400 mt-1">{enhancement.toolCount} tools active</div>}
                        </button>
                      );
                    })}
                    <button
                      onClick={() => { setModal(null); setShowAgentBrowser(true); }}
                      className="col-span-2 p-4 bg-slate-700 rounded-2xl border border-dashed border-slate-500 hover:border-indigo-500 transition-all text-center"
                    >
        ...
        {showAccessGranted && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-slate-700 border border-emerald-500/30 rounded-3xl p-8 max-w-md w-full text-center ">
              ...
              <div className="space-y-2 text-left text-sm text-white mb-6">
  ...
  return (
    <button 
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
        active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-700 hover:text-white'
      }`}
    >
...
function Header({ user, notifications }: { user: any, notifications: Array<any> }) {
  ...
  return (
    <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-white">Welcome back, {user.name || 'User'}! 👋</h1>
            <div className="flex items-center gap-3 mt-1">
              <p className="text-white text-xs font-bold uppercase tracking-widest">{new Date().toLocaleDateString('en-NG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} • {time}</p>
              <div className="h-3 w-px bg-slate-600"></div>
              <p className="text-orange-400 text-xs font-bold uppercase tracking-widest">TIN: 2512403526652</p>
            </div>
          </div>
      <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all border ${
              pushEnabled
                ...
                : 'bg-slate-700 border-slate-500 text-slate-400 hover:text-white'
            }`}
        ...
        <div className="relative">
          <button 
            ...
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all border ${
              showNotifs ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg' : 'bg-slate-700 border-slate-500 text-slate-400 hover:text-white'
            }`}
          >
            🔔
          </button>
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 text-xs text-white flex items-center justify-center rounded-full border-2 border-slate-950 font-black shadow-lg">
              {unreadCount}
            </span>
          )}

          {showNotifs && (
            <div className="absolute right-0 mt-4 w-80 bg-slate-700 border border-slate-500 rounded-3xl shadow-2xl z-50 overflow-hidden ">
              <div className="p-4 border-b border-slate-500 bg-slate-700/80 flex justify-between items-center">
                <h3 className="font-black text-xs uppercase tracking-widest">Notifications</h3>
                <button className="text-xs font-black text-indigo-400 uppercase">Mark all read</button>
              </div>
              <div className="max-h-96 overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="p-8 text-center text-white text-xs italic">No new notifications</div>
                ) : (
                  notifications.map(n => (
                    <div key={n._id} className={`p-4 border-b border-slate-500 hover:bg-slate-700/30 transition-colors cursor-pointer ${!n.read ? 'bg-indigo-600/5' : ''}`}>
              ...
              <button className="w-full py-3 bg-slate-700 text-xs font-black uppercase tracking-widest hover:bg-slate-700 transition-all">View All Notifications</button>
            </div>
          )}
        </div>
        <div className="flex items-center gap-3 pl-4 border-l border-slate-500 group relative cursor-pointer">
          <div className="text-right hidden md:block">
            <p className="text-sm font-black leading-none">{user.name || 'Account'}</p>
            <p className="text-xs text-white font-bold uppercase tracking-widest">{user.email}</p>
          </div>
          ...
          {/* Avatar Dropdown */}
          <div className="absolute right-0 top-full mt-4 w-48 bg-slate-700 border border-slate-500 rounded-2xl shadow-2xl z-50 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0">
             <div className="p-2 space-y-1">
                <button className="w-full text-left px-4 py-3 text-xs font-bold text-slate-400 hover:bg-slate-700 hover:text-white rounded-xl transition-colors">Profile Settings</button>
                <button className="w-full text-left px-4 py-3 text-xs font-bold text-slate-400 hover:bg-slate-700 hover:text-white rounded-xl transition-colors">Billing & Plans</button>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-slate-700 border border-slate-500 rounded-2xl p-6">
              <h3 className="text-xl font-black text-white mb-6 flex items-center gap-2">📈 Subscription Value</h3>
              <div className="h-64">
            ...
            <div className="bg-slate-700 border border-slate-500 rounded-2xl p-6">
              <h3 className="text-xl font-black text-white mb-6 flex items-center gap-2">🧩 Agent Usage</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <Section title="Recent Projects" actionLabel="View All">
            <div className="bg-slate-700/80 border border-slate-500 rounded-2xl overflow-hidden overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-700/80 border-b border-slate-500">
                  <tr>
                    <th className="px-6 py-4 font-black text-white uppercase text-xs">Project</th>
                    <th className="px-6 py-4 font-black text-white uppercase text-xs">Agent</th>
                    <th className="px-6 py-4 font-black text-white uppercase text-xs">Status</th>
                    <th className="px-6 py-4 font-black text-white uppercase text-xs">Format</th>
                    <th className="px-6 py-4 font-black text-white uppercase text-xs">Date</th>
                ...
                <tbody className="divide-y divide-slate-800">
                  {data.projects.map((p: any) => (
                    <tr key={p._id} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-white">{p.format}</td>
                  ...
                  {data.projects.length === 0 && (
                    <tr><td colSpan={6} className="px-6 py-8 text-center text-white italic">No projects yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-700 border border-slate-500 rounded-2xl p-6">
            ...
            <div className="flex gap-2">
              <input readOnly value={`prosuite.ng/ref/${data.user.referralCode}`} className="flex-grow bg-slate-900 border border-slate-500 rounded-lg px-3 py-2 text-xs font-mono text-white focus:outline-none" />
              <button onClick={() => {
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="text-xs text-white">Earned: <span className="text-slate-100 font-bold">₦{data.referrals.totalEarned}</span></div>
...
function StatCard({ label, value, icon, color }: { label: string, value: any, icon: string, color: string }) {
  ...
  return (
    <div className="p-5 bg-slate-700 border border-slate-500 rounded-2xl shadow-lg">
      <div className={`w-12 h-12 rounded-xl mb-3 flex items-center justify-center text-2xl border ${colors[color]}`}>
        {icon}
      </div>
      <p className="text-sm text-white font-bold uppercase tracking-wider mb-1">{label}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {actionLabel && <button className="text-sm text-indigo-400 font-bold hover:underline">{actionLabel} →</button>}
      </div>
      {children}
    </div>
  );
...
function SubscriptionRow({ sub }: { sub: any }) {
  ...
  return (
    <div className="p-4 bg-slate-700 border border-slate-500 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-indigo-500/50 transition-all">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded font-black uppercase">{priceMap[sub.plan] || "₦12,500/mo"}</span>
            <span className="text-xs text-white italic">Next: {new Date(sub.endsAt).toLocaleDateString()}</span>
          </div>
        </div>
      </div>
      <div className="flex-grow max-w-md mx-4">
        <div className="flex justify-between text-xs uppercase font-black tracking-widest text-white mb-2">
          <span>{daysRemaining} Days remaining</span>
          <span className="text-emerald-400">ACTIVE ✅</span>
        </div>
        <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden border border-slate-500/50">
      ...
      <div className="flex gap-2">
        <button className="px-4 py-2 bg-slate-700 text-xs font-black uppercase tracking-widest rounded-lg hover:bg-slate-700 transition-all border border-slate-500">Pause</button>
        <button className="px-4 py-2 bg-indigo-600 text-xs font-black uppercase tracking-widest rounded-lg hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/20">Upgrade</button>
      </div>
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2 py-1 rounded-md text-xs font-bold border uppercase tracking-tighter ${styles[status] || styles["in-progress"]}`}>
      {status}
    </span>
  );
}
...
function EmptyState({ icon, text }: { icon: string, text: string }) {
  return (
    <div className="p-12 text-center border-2 border-dashed border-slate-500 rounded-2xl">
      ...
      <Section title="Payment Methods">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.paymentMethods.map((pm: any) => (
            <div key={pm._id} className="p-6 bg-slate-700 border border-slate-500 rounded-2xl relative group">
              <div className="flex items-center justify-between mb-4">
                <div className="text-2xl">{pm.type === 'card' ? '💳' : '🏦'}</div>
                {pm.isDefault && <span className="text-xs bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded font-bold uppercase">Default</span>}
              </div>
              <p className="font-bold text-lg">{pm.provider} {pm.last4 ? `•••• ${pm.last4}` : ''}</p>
              <p className="text-xs text-white mt-1 uppercase tracking-widest font-medium">Expires 04/28</p>
              <div className="mt-4 flex gap-2">
                {!pm.isDefault && <button className="text-xs font-bold text-indigo-400 hover:underline uppercase">Set Default</button>}
                <button className="text-xs font-bold text-red-500 hover:underline uppercase ml-auto">Remove</button>
              </div>
            </div>
          ))}
          <button className="p-6 border-2 border-dashed border-slate-500 rounded-2xl flex flex-col items-center justify-center text-white hover:border-slate-500 hover:text-slate-400 transition-all gap-2">
    ...
    <div className="space-y-8 ">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Section title="Referral History">
          <div className="bg-slate-700 border border-slate-500 rounded-2xl overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-700/80 border-b border-slate-500">
                <tr>
                  <th className="px-6 py-4 font-semibold text-white">Friend</th>
                  <th className="px-6 py-4 font-semibold text-white">Date</th>
                  <th className="px-6 py-4 font-semibold text-white">Commission</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {data.referrals.history.map((r: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td className="px-6 py-4 text-white">{new Date(r.date).toLocaleDateString()}</td>
        ...
        <Section title="Withdraw Funds">
          <div className="bg-slate-700 border border-slate-500 rounded-2xl p-8 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div>
              <p className="text-white font-bold uppercase tracking-widest text-xs mb-2">Withdrawable Balance</p>
              <h2 className="text-4xl font-black">₦{data.referrals.availableBalance}</h2>
            </div>
            <div className="grid grid-cols-2 gap-2 w-full max-w-xs">
              <button className="py-3 bg-slate-700 rounded-xl text-xs font-bold hover:bg-slate-700 border border-slate-500">Bank Transfer</button>
              <button className="py-3 bg-slate-700 rounded-xl text-xs font-bold hover:bg-slate-700 border border-slate-500">OPay / Palmpay</button>
            </div>
            ...
            <p className="text-xs text-white uppercase font-bold tracking-tighter">Minimum withdrawal: ₦1,000 • Processed every Friday 2 PM</p>
          </div>
        </Section>
      </div>
    </div>
  ...
  return (
    <div className="space-y-8 ">
      <div className="flex justify-between items-center">
        ...
        <button className="px-6 py-2 bg-red-600/10 border border-red-600/20 text-red-500 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-red-600 hover:text-white transition-all">Logout Remote Devices</button>
      </div>
      
      <Section title="Active Personnel Sessions">
        <div className="bg-slate-700 border border-slate-500 rounded-3xl overflow-hidden shadow-2xl">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-700/80 border-b border-slate-500">
              <tr>
                <th className="px-8 py-5 font-black uppercase text-xs text-white tracking-widest">Device / OS</th>
                <th className="px-8 py-5 font-black uppercase text-xs text-white tracking-widest">Geographical Data</th>
                <th className="px-8 py-5 font-black uppercase text-xs text-white tracking-widest">Network IP</th>
                <th className="px-8 py-5 font-black uppercase text-xs text-white tracking-widest">Last Pulse</th>
                <th className="px-8 py-5 font-black uppercase text-xs text-white tracking-widest text-right">Session State</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {data.sessions.map((s: any) => (
                <tr key={s._id} className={`${s.isCurrent ? "bg-indigo-600/5" : ""} hover:bg-slate-700/30 transition-colors`}>
                  <td className="px-8 py-5 font-bold flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white uppercase font-bold">Authorized Session</p>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-slate-400 font-bold">{s.location}</td>
                  <td className="px-8 py-5 text-white font-mono text-xs tracking-tighter">{s.ip}</td>
                  <td className="px-8 py-5 text-white font-bold">{new Date(s.lastActive).toLocaleTimeString()}</td>
                  <td className="px-8 py-5 text-right">
                    {s.isCurrent ? (
                      <span className="text-emerald-400 font-black uppercase text-xs border border-emerald-500/20 px-3 py-1.5 rounded-lg bg-emerald-500/5 shadow-lg shadow-emerald-500/10">Current Session</span>
                    ) : (
                      <button className="text-red-500 font-black uppercase text-xs hover:underline tracking-widest">Revoke Access</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="p-8 bg-slate-700 border border-slate-500 rounded-2xl space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl border border-slate-500">
            ...
            <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl border border-slate-500">
              ...
              <span className="text-sm font-black text-white">0 Active</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl border border-slate-500">
              <span className="text-xs text-slate-400 font-bold uppercase">Last Verification</span>
              <span className="text-xs font-bold text-white">2 min ago</span>
            </div>
          </div>
        </div>
        <div className="p-8 bg-slate-700 border border-slate-500 rounded-2xl space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-700/80 rounded-xl border border-slate-500">
            <div>
              <p className="font-bold">Authenticator App</p>
              <p className="text-xs text-white italic">Google Authenticator, Authy, etc.</p>
            </div>
        ...
        <div className="p-8 bg-slate-700 border border-slate-500 rounded-2xl space-y-6">
          ...
          <form className="space-y-4" onSubmit={async (e) => {
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase font-black text-white tracking-widest">Current Password</label>
              <input type="password" placeholder="••••••••" className="w-full bg-slate-700 border border-slate-500 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
            </div>
            <div className="space-y-2">
              <label className="text-xs uppercase font-black text-white tracking-widest">New Password</label>
              <input type="password" placeholder="••••••••" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full bg-slate-700 border border-slate-500 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
            </div>
            ...
            <button type="submit" className="w-full py-3 bg-slate-700 rounded-xl text-sm font-black border border-slate-500 hover:bg-slate-700">UPDATE PASSWORD</button>
          </form>
        </div>
      </div>
    </div>
...
function Settings({ data }: { data: any }) {
  return (
    <div className="space-y-8 ">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-slate-700 border border-slate-500 rounded-2xl p-8 space-y-6">
          <h3 className="text-xl font-bold flex items-center gap-2">👤 Profile Settings</h3>
          <div className="space-y-4">
            <div className="flex items-center gap-6 p-4 bg-slate-700/30 rounded-xl border border-slate-500">
              ...
              <div>
                ...
                <p className="text-xs text-white uppercase font-bold tracking-widest">Max size 2MB • PNG/JPG</p>
              </div>
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-700 border border-slate-500 rounded-2xl p-8 space-y-6">
            ...
            <div className="flex gap-2">
              ...
              <button className="flex-1 py-3 bg-slate-700 rounded-xl font-bold border border-slate-500 text-slate-400">USD ($)</button>
            </div>
          </div>
        </div>
      </div>
...
function Input({ label, value, placeholder, disabled }: { label: string, value?: string, placeholder?: string, disabled?: boolean }) {
  return (
    <div className="space-y-1.5">
      <label className="text-xs uppercase font-bold text-white tracking-widest">{label}</label>
      <input 
        defaultValue={value}
        placeholder={placeholder}
        disabled={disabled}
        className="w-full bg-slate-900 border border-slate-500 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all disabled:opacity-50 text-sm" 
      />
    </div>
  );
}
...
function Toggle({ label, description, active }: { label: string, description: string, active?: boolean }) {
  return (
    <div className="flex items-center justify-between p-4 bg-slate-700/20 rounded-xl border border-slate-500">
      <div className="max-w-[80%]">
        <p className="text-sm font-bold">{label}</p>
        <p className="text-xs text-white leading-tight">{description}</p>
      </div>
...
function Footer() {
  return (
    <footer className="bg-slate-700 border-t border-slate-500 p-8 mt-auto">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="text-center md:text-left">
          <p className="text-sm font-bold text-white">Powered by Dutchkem Ventures ProSuite NG+</p>
          <p className="text-xs text-white mt-1 uppercase tracking-tighter">© 2026 Dutchkem Ventures. RC: 9489855. All rights reserved.</p>
        </div>
...
function SocialIcon({ icon }: { icon: string }) {
  return (
    <button className="w-8 h-8 rounded-lg bg-slate-700 border border-slate-500 flex items-center justify-center hover:bg-slate-700 transition-all grayscale hover:grayscale-0">
      {icon}
    </button>
  );
}
...
function BuyCreditsModal({ user, onClose }: { user: any; onClose: () => void }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <button
            ...
            className={`w-full p-4 rounded-2xl border text-left transition-all ${
              selectedPlan === plan.id
                ...
                : "bg-slate-700 border-slate-500 hover:border-indigo-500"
      ...
      <p className="text-xs text-white uppercase font-bold text-center">Secure payment powered by Kora Pay</p>
    </div>
  );
}
