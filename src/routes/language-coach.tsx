...
export const Route = createFileRoute('/language-coach')({
  component: LanguageCoachPage,
})

function LanguageCoachPage() {
  const { isAuthenticated, isLoading } = useConvexAuth();
  const navigate = useNavigate();
  const [threadId, setThreadId] = useState<string | null>(null);
  const createThread = useMutation(api.language_chat.createThread);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans">
      <AgentHeader agentName="Language Coach" />
...
function ChatContainer({ threadId }: { threadId: string }) {
  const { results, status, loadMore } = useUIMessages(
    api.language_chat.listMessages,
    { threadId },
    ...
    <div className="flex-grow flex flex-col h-full">
      ...
      >
        ...
        {results.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-6">
            <div className="w-20 h-20 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-3xl flex items-center justify-center text-5xl shadow-2xl shadow-indigo-400/20">🗣️</div>
            <div>
              <h3 className="text-2xl font-bold text-white mb-2 uppercase tracking-tight">Global Linguistic Studio</h3>
      ...
      <div className="p-4 md:p-8 pt-0">
        <div className="flex items-center gap-2">
          ...
          <AgentSupportButton agentId="A11" agentName="Language Tutor" />
        </div>
        <p className="text-[10px] text-center text-slate-600 mt-3 uppercase tracking-widest font-black">
          Dutchkem ProSuite NG+ Language Agent • Native Fluency Llama 3.3
        </p>
      </div>
    </div>
  );
    ...
    <div className={`flex ${isAssistant ? 'justify-start' : 'justify-end'} group animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      <div className={`max-w-[85%] md:max-w-[70%] flex gap-3 ${isAssistant ? '' : 'flex-row-reverse'}`}>
        <div className={`w-10 h-10 rounded-lg flex-shrink-0 flex items-center justify-center text-base font-bold ${
          isAssistant ? 'bg-gradient-to-br from-violet-500 to-purple-500 text-white' : 'bg-violet-600 text-white'
        }`}>
          {isAssistant ? '🗣️' : 'U'}
        </div>
        <div className={`p-4 rounded-2xl ${
          ...
          <div className="whitespace-pre-wrap leading-relaxed text-[15px] leading-7">
            ...
            {message.status === "streaming" && (
              <span className="inline-block w-1.5 h-4 ml-1 bg-violet-400 animate-pulse align-middle"></span>
            )}
          </div>
          {isAssistant && visibleText && visibleText.length > 50 && message.status !== "streaming" && (
            <div className="mt-3 pt-3 border-t border-slate-700">
              <FileDownloadButtons content={visibleText} agentType="Language Coach" title="Language Output" />
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="relative group">
      <input
        ...
        placeholder="Translate something or start a conversation..."
        className="w-full bg-slate-900 border border-slate-700 text-white rounded-2xl px-6 py-5 pr-16 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all placeholder:text-slate-600 shadow-2xl font-medium"
      />
      <button 
        type="submit"
        disabled={!input.trim()}
        className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 bg-indigo-600 rounded-xl flex items-center justify-center text-white hover:scale-105 transition-all disabled:opacity-50 disabled:grayscale"
      >
