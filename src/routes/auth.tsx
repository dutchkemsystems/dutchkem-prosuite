...
function AuthTabs() {
  ...
  return (
    <div>
      <div className="flex gap-2 mb-6 p-1 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20">
        <button
          onClick={() => setTab('google')}
          className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tab === 'google' ? 'bg-white text-slate-900' : 'text-white/70 hover:text-white'}`}
        >
          Google
        </button>
        <button
          onClick={() => setTab('email')}
          className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tab === 'email' ? 'bg-white text-slate-900' : 'text-white/70 hover:text-white'}`}
        >
          Email OTP
        </button>
      </div>
      {tab === 'google' ? <GoogleSignInButton /> : <EmailAuthForm />}
    </div>
  );
}
...
function EmailAuthForm() {
  ...
  if (step === "email") {
    return (
      <form onSubmit={handleEmailSubmit} className="bg-slate-900 border border-slate-800 p-10 rounded-[2.5rem] shadow-2xl ">
        <div className="space-y-6">
          <div>
