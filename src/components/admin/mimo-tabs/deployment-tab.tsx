...
export function DeploymentTab({ adminToken, isDark, setCommandOutput }: DeploymentTabProps) {
  ...
  const runDeploy = async (platform: string, type: string) => { setCommandOutput('Deploying to ' + platform + '...'); try { const r = await deployMutation({ adminToken, platform: platform as any, type: type as any }); setCommandOutput(JSON.stringify(r, null, 2)); } catch (e: any) { setCommandOutput('Error: ' + e.message); } }
